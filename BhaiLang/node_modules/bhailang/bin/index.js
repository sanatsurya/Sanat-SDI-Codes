    ...
    var BinaryExpression = class {
      visitNode(node) {
        ...
        if (node.type == import_bhai_lang_parser.NodeType.BinaryExpression) {
          ...
          this._checkNalla(node);
          left2 = node.left.type == import_bhai_lang_parser.NodeType.BooleanLiteral ? node.left.value == "sahi" ? true : false : InterpreterModule.getVisitor(node.left.type).visitNode(node.left);
          right2 = node.right.type == import_bhai_lang_parser.NodeType.BooleanLiteral ? node.right.value == "sahi" ? true : false : InterpreterModule.getVisitor(node.right.type).visitNode(node.right);
        }
